...
<div class="row">
  <div class="col-md-12">
    @include('admin.partials.ui.card_start', [
      'title' => ($category_group->hasTranslation($selected_language) ? trans('app.update_model_translation', ['model' => trans('app.model.category_group')]) : trans('app.add_model_translation', ['model' => trans('app.model.category_group')])) . ' | ' . $category_group->name,
      'icon' => 'fa-language',
      'class' => 'admin-form-section',
      'actions' => '<a href="javascript:void(0)" data-link="' . route('admin.catalog.categoryGroup.translate.bulk') . '" class="ajax-modal-btn btn btn-default btn-flat btn-sm"><em class="fa fa-language"></em> ' . e(trans('app.bulk_translation_import')) . '</a>',
    ])
      ...
      <button type="submit" class="btn btn-flat btn-lg btn-new">{{ $category_group->hasTranslation($selected_language) ? trans('app.form.update') : trans('app.form.save') }}</button>
    </div>
