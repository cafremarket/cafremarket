@php
  $translation = $category_translation->translation ?? null;
@endphp

{{ Form::hidden('lang', $selected_language) }}
@include('admin.partials.ui.card_start', [
  'title' => ($category->hasTranslation($selected_language) ? trans('app.update_model_translation', ['model' => trans('app.model.category')]) : trans('app.add_model_translation', ['model' => trans('app.model.category')])) . ' | ' . $category->name,
  'icon' => 'fa-language',
  'class' => 'admin-form-section',
        ...
        <ul class="nav nav-tabs nav-tabs-justified mb-4">
          @foreach ($available_languages as $lang)
            <li class="col-md-{{ $loop->count > 1 ? 12 / $loop->count : 12 }} pr-0 pl-0 {{ $selected_language == $lang->code ? 'active' : '' }} text-center">
              <a href="{{ route('admin.catalog.category.translate.form', ['category' => $category, 'language' => $lang->code]) }}">
    ...
    <div class="text-right">
      <button type="submit" class="btn btn-flat btn-lg btn-new">{{ $category->hasTranslation($selected_language) ? trans('app.form.update') : trans('app.form.save') }}</button>
    </div>
