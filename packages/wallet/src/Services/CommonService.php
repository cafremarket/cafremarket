...
class CommonService
{
     ...
     * @param  int  $amount
     */
    public function transfer($from, $to, $amount, ?array $meta = null, string $status = TransferStatuses::STATUS_TRANSFER): Transfer
    {
     ...
     * @param  int  $amount
     */
    public function forceTransfer($from, $to, $amount, ?array $meta = null, string $status = TransferStatuses::STATUS_TRANSFER): Transfer
    {
        return app(LockService::class)->lock($this, __FUNCTION__, function () use ($from, $to, $amount, $meta, $status) {
            $math = app(Mathable::class);
    ...
    public function verifyWithdraw($wallet, $amount, ?bool $allowZero = null): void
    {
        ...
        if ($amount && ! $wallet->balance) {
            throw new BalanceIsEmpty(trans('packages.wallet.wallet_empty'));
        }

        if (! $wallet->canWithdraw($amount, $allowZero)) {
            throw new InsufficientFunds(trans('packages.wallet.insufficient_funds'));
        }
    }
