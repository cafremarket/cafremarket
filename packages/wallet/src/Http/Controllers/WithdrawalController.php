...
class WithdrawalController extends Controller
{
    ...
    public function form(Request $request)
    {
        ...
        $locked_account = $this->shop instanceof Shop ? ShopPayoutAccount::summary($this->shop) : null;

        if (Auth::guard('affiliate')->check()) {
            $existing_instruction = Auth::guard('affiliate')->user()->pay_to;
        }

        return view('wallet::_withdraw', compact('balance', 'minimum', 'existing_instruction', 'locked_account'));
    }
    ...
    public function withdraw(WithdrawalRequest $request)
    {
        if ($this->shop instanceof Shop) {
            // Vendors withdraw to one fixed account: registered on the first request, then locked.
            $account = ShopPayoutAccount::resolveForWithdrawal($this->shop, $request);
