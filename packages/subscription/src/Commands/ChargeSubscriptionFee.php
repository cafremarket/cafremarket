                ...
                if ($subscription->provider == 'wallet') {
                    ...
                    if ($owner) {
                        Log::channel('subscription')->info(
                            'Checking subscription for shop: ' . $subscription->owner->name
                        );
                    } else {
                        Log::channel('subscription')->error(
                            'Checking subscription FAILED! The shop is not found in the database. Shop ID= ' . $subscription->shop_id
                        );
                    ...
                    if ($subscription->onTrial()) {
                        Log::channel('subscription')->info('Subscription onTrial');
                        $real_expiry_time = $subscription->trial_ends_at;
                    } elseif ($subscription->onGracePeriod()) {
                        Log::channel('subscription')->info('Subscription onGracePeriod');
                        $real_expiry_time = $subscription->ends_at;
                    } elseif ($subscription->ended()) {
                        Log::channel('subscription')->info('Subscription ended');
                        $real_expiry_time = $subscription->ends_at;
                    } elseif ($subscription->trial_ends_at && $subscription->trial_ends_at->isPast()) {
                        Log::channel('subscription')->info('Subscription trial ended');
                        $real_expiry_time = $subscription->trial_ends_at;
                    ...
                    if ($real_expiry_time) {
                        Log::channel('subscription')->info('Subscription ending at: ' . $real_expiry_time);
                    }
                    ...
                    if ($real_expiry_time && $real_expiry_time->lte($billing_time)) {
                        try {
                            Log::channel('subscription')->info(
                                'Billing: ' . $subscription->shop_id ?? 'Shop Not Found'
                            );
                            ...
                            Log::channel('subscription')->info('Custom subscription fee: ' . $cost);
                            ...
                            if ($cost == 0) {
                                Log::channel('subscription')->info('cost 0: ' . $cost);
                                ...
                                if ($owner->balance < $cost) {
                                    Log::channel('subscription')->info(
                                        'Subscription charges (shop:: ' . $subscription->shop_id . ') failed beacuse of insufficient balance.'
                                    );
                            ...
                            Log::channel('subscription')->info(
                                'Subscription charges (shop:: ' . $subscription->shop_id . ') failed because of ' . $e->getMessage()
                            );
                        }
                    }
                    ...
                    if (
                        ...
                        Log::channel('subscription')->info('Sending subscription notifications.....');
                        ...
                        try {
                            ...
                            Log::channel('subscription')->info(
                                'Subscription notification sending Error: ' . $e->getMessage()
                            );

                            Log::channel('subscription')->info($e);
                        }
