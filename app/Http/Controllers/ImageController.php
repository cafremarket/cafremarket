...
class ImageController extends Controller
{
    ...
    public function show(Request $request, Server $server, $path)
    {
        ...
        // Uploaded chat/order attachments live under attachments/ — serve the file directly.
    ...
    /**
     * Stream a file from the default disk without Glide processing.
     */
    private function rawStorageFileResponse(string $path)
    {
        if (! $this->disk->exists($path)) {
            abort(404);
        }
    ...
    public function upload(Request $request)
    {
        ...
        $ext = $rawFile->getClientOriginalExtension() === 'svg' ? 'svg' : 'webp';
        ...
        // $ext = $rawFile->getClientOriginalExtension() === 'svg' ? 'svg' : 'webp';
        // $raw_file_path = $rawFile->getRealPath();
        if ($ext == 'svg') {
            $convert = file_get_contents($rawFile->getRealPath());
        } else {
            $convert = InterventionImage::read($rawFile->getRealPath())->toWebp(100)->toFilePointer();
        }
        ...
        if ($totalChunks > 1) {    // create chunk files only if chunks are greater than 1
            ...
            if (move_uploaded_file($convert, $chunkFile)) {
                ...
                // Return to proceed if all chunks are not uploaded yet
