...
class ChartHelper
{
    ...
    public static function Days($days = null, $format = 'F d', $start = null)
    {
        ...
        $data = [];
        for ($i = $days - 1; $i >= 0; $i--) {
            $data[] = $start->copy()->subDays($i)->translatedFormat($format);
        }
    ...
    public static function Months($months = null, $format = 'F', $start = null)
    {
        ...
        for ($i = $months - 1; $i >= 0; $i--) {
            $data[] = $start->copy()->subMonths($i)->translatedFormat($format);
        }
    ...
    public static function visitorsOfMonths($months = null, $start = null)
    {
        ...
        if ($breakdown) {
            $weeks = static::visitorsOfDays($breakdown);
    ...
    public static function getSaleAmount($timeframe, $value = false)
    {
        ...
        if ($timeframe === 'week' || $timeframe === 'month') {
            while ($startDate <= $endDate) {
                if ($timeframe === 'week') {
                    $dayOfTimeframe = $startDate->translatedFormat('D'); // Get the day of the week (e.g., "Sat", "Sun", etc.)
                } elseif ($timeframe === 'month') {
                    $dayOfTimeframe = $startDate->translatedFormat('d'); // Get the day of the month (e.g., "01", "02", etc.)
                }
            ...
            while ($startDate <= $endDate) {
                $monthOfYear = $startDate->translatedFormat('M'); // Get the month abbreviation (e.g., "Jan", "Feb", etc.)
