...
if (! function_exists('get_sales_commission_for_order')) {
    /**
     * Marketplace commission for an order (deducted when crediting the vendor wallet).
     * Plan transaction_fee is charged to the customer, not here.
     */
    function get_sales_commission_for_order($order): float
    {
        return (float) get_marketplace_commission_for_order($order);
    }
}
...
if (! function_exists('getPlatformFeeForOrder')) {
    ...
    {
        return get_marketplace_commission_for_order($order);
    }
}
...
if (! function_exists('can_use_own_catalog_only')) {
    /**
     * Check if the catalog system enabled and set to use own catalog only
     */
    function can_use_own_catalog_only()
    {
        // Always true when the catalog is not enabled
        if (! is_catalog_enabled()) {
            return true;
        }

        return (bool) config('system_settings.can_use_own_catalog_only', true);
    }
}
