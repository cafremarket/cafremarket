...
class EnvManager
{
    ...
    public function setValue(string $key, string $value, $write = true): string
    {
        ...
        // When have to write on .env file
        if ($write) {
            $this->write();

            Log::info("Evironemnt key {$key} has been updated to {$value}");
        }
    ...
    {
        if (strpos($value, ' ') !== false || (strlen($value) && in_array($value[0], ['=', '$']))) {
            $value = '"'.$value.'"';
        }
    ...
    {
        $this->_saved = (file_put_contents($this->_envPath, $this->_envContent) !== false ?? true);
